...
# Extracts data about the FNC dataset
...
print(len(fnc_labels_test))

labels = [0, 0, 0, 0]
for label in fnc_labels_test:
    labels[label] += 1

print(labels)

avg_len = 0.0
for headline in fnc_headlines_test:
    avg_len += len(headline.split())

print(avg_len/len(fnc_headlines_test))

avg_body_len = 0.0
for body in fnc_bodies_test:
    avg_body_len += len(body.split())
    
print(avg_body_len/len(fnc_bodies_test))
